...
namespace structures
{
	...
	inline PriorityQueueList<T>& PriorityQueueList<T>::operator=(const PriorityQueueList<T>& other)
	{
		...
		{
			clear();
			for (PriorityQueueItem<T>* p : *other.list_) {
				list_->add(new PriorityQueueItem<T>(*p));
			}
		}
		return *this;
	}
	...
	{
		for (PriorityQueueItem<T>* p : *list_) {
			delete p;
		}
		list_->clear();
	}
	...
	{
		PriorityQueueItem<T>* p = list_->removeAt(indexOfPeek());
		T result = p->accessData();
		delete p;
		return result;
	}
