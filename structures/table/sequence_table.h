...
namespace structures 
{
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::operator=(const SequenceTable<K, T>& other)
	{
		if (this != &other) {
			clear();
			for (TableItem<K,T>* var : *other.list_) {
				list_->add(new TableItem<K,T>(*var));
			}
		}
		return *this;
	}
	...
	inline T & SequenceTable<K, T>::operator[](const K key)
	{
		...
		throw std::logic_error("Hranate zatvorky chyba");
	}
	...
	{
		if (containsKey(key)) {
			throw std::logic_error("Insert Tabulka Sequencna");
		}
	...
	{
		TableItem<K,T>* var = findTableItem(key);
		if (var == nullptr) {
			throw std::logic_error("Remove Tabulka Sequencna");
		}
		list_->tryRemove(var);
		T data = var->accessData();
		delete var;
		return data;
	}
